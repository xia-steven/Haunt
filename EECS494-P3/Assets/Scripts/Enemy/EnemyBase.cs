...
namespace Enemy {
    ...
    public class EnemyBase : MonoBehaviour {
                ...
                // Confirm that the raycast did not hit the player
                    ...
                    switch (runningCoroutine) {
                        case false:
                            ...
                            Pathfinding.PathfindingController.FindClosestWalkable(targetPosition, out var x, out var y);
        ...
        protected virtual IEnumerator EnemyAttack() {
            ...
            while (state == EnemyState.Attacking) yield return null;
        ...
        private IEnumerator MoveWithAStar(int targetX, int targetZ) {
            ...
            while (state == EnemyState.AStarMovement) {
                if (pathVectorList != null) {
                    if (currentPathIndex >= pathVectorList.Count) currentPathIndex = pathVectorList.Count - 1;

                    var targetPosition =
                        pathVectorList[currentPathIndex] + Pathfinding.PathfindingController.map.origin;
        ...
        private IEnumerator updatePathfindingVector(int targetX, int targetZ) {
            while (state == EnemyState.AStarMovement) {
                // Set player position
                pathVectorList = Pathfinding.AStar.Instance.FindPath(transform.position, targetX, targetZ);
        ...
        private void OnTriggerEnter(Collider other) {
            if (other.gameObject.CompareTag("Player")) playerHealth.AlterHealth(-1, DeathCauses.Enemy);
        }
    }
