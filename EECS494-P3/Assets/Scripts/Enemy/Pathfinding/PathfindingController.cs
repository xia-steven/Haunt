...
namespace Enemy.Pathfinding {
    ...
    public class PathfindingController : MonoBehaviour {
        ...
        private void Start() {
            ...
            aStar = new AStar(map.dimension.x, map.dimension.y, map.origin);
            foreach (var tile in map.unwalkableTiles) aStar.GetNode(tile.x, tile.y).SetIsWalkable(false);
        }

        public static void FindClosestWalkable(Vector3 worldPosition, out int targetX, out int targetZ) {
            var grid = aStar.GetGrid();
